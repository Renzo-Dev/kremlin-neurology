  ...
  setup(props, { emit }) {
    ...
    const handleFileSelect = (event) => {
      ...
      if (file) {
        ...
        fileError.value = ''
        console.log('✅ File set to selectedFile:', selectedFile.value)
        
        // Автозаполнение названия если пустое
        if (!formData.value.title || formData.value.title.trim() === '') {
          const fileNameWithoutExtension = file.name.replace(/\.[^/.]+$/, '')
          formData.value.title = fileNameWithoutExtension
          console.log('📝 Auto-filled title:', fileNameWithoutExtension)
        } else {
          console.log('📝 Title already exists, keeping:', formData.value.title)
        }
      }
    }

    const removeFile = () => {
      console.log('🗑️ Remove file clicked, current selectedFile:', selectedFile.value)
      ...
      if (wasFileSelected && formData.value.title) {
        ...
        console.log('📝 Title cleared because file was removed')
      }
      
      console.log('✅ File removed, selectedFile is now:', selectedFile.value)
    }
    ...
    const validateForm = () => {
      ...
      if (!formData.value.title || !formData.value.title.trim()) {
        ...
        console.log('✅ Title validation passed:', formData.value.title)
      }
      
      // Проверка авторов
      if (!formData.value.authors || !formData.value.authors.trim()) {
        errors.value.authors = 'Авторы обязательны'
        console.log('❌ Authors validation failed - empty or whitespace')
      } else {
        console.log('✅ Authors validation passed:', formData.value.authors)
      }
      ...
      console.log('  - File error:', fileError.value)
      console.log('  - Has errors:', hasErrors)
      console.log('  - Has file error:', hasFileError)
      console.log('  - Final result:', !hasErrors && !hasFileError)
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        console.log('📤 Emitting save event with data:', fileData)
        console.log('📊 Final category:', finalCategory.value)
        console.log('📁 File object:', selectedFile.value)
