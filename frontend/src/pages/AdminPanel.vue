...
export default {
  ...
  setup() {
    ...
    const loadFiles = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          const allFiles = []
          // catalogData.items содержит файлы, сгруппированные по первой букве фамилии автора
          // Нужно извлечь все файлы из всех групп
          Object.keys(catalogData.items || {}).forEach(authorLetter => {
            if (Array.isArray(catalogData.items[authorLetter])) {
              console.log(`📁 Processing group '${authorLetter}':`, catalogData.items[authorLetter])
              catalogData.items[authorLetter].forEach(file => {
                // Сохраняем оригинальные данные файла, включая category
          ...
          console.log('📚 All files extracted:', allFiles)
    ...
    const handleFilter = () => {
      console.log('🔧 Filter applied - Category:', selectedCategory.value)
      currentPage.value = 1
      updatePagination()
    }

    const handleSort = () => {
      console.log('📊 Sort applied - By:', sortBy.value)
      currentPage.value = 1
      updatePagination()
    }
    ...
    const handleFileSave = async (fileData) => {
      try {
        ...
        console.error('❌ Error in handleFileSave:', err)
